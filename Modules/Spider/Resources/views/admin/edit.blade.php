    ...
    <div class="card card-info">
        <form id="add-form" class="form-horizontal form-ajax card-body" role="form" data-toggle="validator" method="POST" action="">
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <input type="text" class="form-control" id="url" name="row[url]" placeholder="" value="{{$info->url ?? ''}}" data-rule="url" />
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <input type="text" class="form-control" id="domain_prefix" name="row[domain_prefix]" placeholder="" value="{{$info->domain_prefix ?? ''}}" data-rule="" />
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <label>
                        <input type="radio" name="row[sub_tasks]" value="0" class="flat-red" @if ($info->sub_tasks == 0) checked @endif> 否
                    </label>
                    <label>
                        <input type="radio" name="row[sub_tasks]" value="1" class="flat-red" @if ($info->sub_tasks == 1) checked @endif> 是
                    </label>
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <input type="text" class="form-control" id="timer" name="row[timer]" placeholder="" value="{{$info->timer ?? '* * * * *'}}" data-rule="" data-tips="bottom" title="使用下面的cron选择器选择" readonly/>
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <select class="form-control select2  col-xs-12 col-sm-12" name="row[type]" style="border-radius:0px;" >
                        <option value="1" @if($info['type']==1) selected @endif>1文章正文</option>
                        <option value="2" @if($info['type']==2) selected @endif>2文章列表</option>
                        <option value="3" @if($info['type']==3) selected @endif>3报刊</option>
                        <option value="4" @if($info['type']==4) selected @endif>4其他</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    @foreach ($info['rules'] as $filed => $rules)
                        @php
                            $field_handle = isset($rules['field_handle']) ?(int)$rules['field_handle']:1;
                            $ruleList = empty($rules['route'])? $rules : $rules['route'];
                            $ruleList = is_array($ruleList)? $ruleList : (array)$ruleList;
                            $isFirstRule = $loop->first;
                            $regExpList = (empty($rules['extend']) || empty($rules['extend']['first']))? ['first'=>'','href'=>''] : $rules['extend'];
                        @endphp
                        @foreach ($ruleList as $key => $rule)
                            <div class="row custom_row_item" style="margin:0;">
                                ...
                                <div class="col-sm-2 p-0">
                                    <button type="button" class="btn btn-success add_custom_row_item" @if($key > 0) style="display: none;" @endif><i class="fa fa-plus" aria-hidden="true"></i></button>
                                    <button type="button" class="btn btn-danger trash_custom_row_item" data-key="{{$key}}" @if($key < 1 && $isFirstRule) style="display: none;" @endif><i class="fa fa-trash" aria-hidden="true"></i></button>
                                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <label>
                        <input type="radio" name="row[url_can_repeated]" value="0" class="flat-red" @if ($info->url_can_repeated == 0) checked @endif> 不能重复采集
                    </label>
                    <label>
                        <input type="radio" name="row[url_can_repeated]" value="1" class="flat-red" @if ($info->url_can_repeated == 1) checked @endif> 能重复采集
                    </label>
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <select class="form-control select2  col-xs-12 col-sm-12" name="row[success][save]" style="border-radius:0px;" >
                        <option value="default" @if($info['success']['save']=='default') selected @endif>默认保存到文章数据</option>
                        <option value="custom" @if($info['success']['save']=='custom') selected @endif>自定义</option>
                    </select>
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <select class="form-control select2  col-xs-12 col-sm-12" name="row[next_tasks_id]" style="border-radius:0px;" >
                        ...
                        @foreach ($sub_tasks as $subTask)
                            <option value="{{ $subTask['id'] }}" @if($info['next_tasks_id'] == $subTask['id']) selected @endif>「{{$subTask['id']}}」:{{ $subTask['name'] }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-xs-12 col-sm-10">
                    <select class="form-control select2  col-xs-12 col-sm-12" name="row[extend][classify_id]" style="border-radius:0px;" >
                        ...
                        @foreach ($classify_list as $classify)
                            <option value="{{ $classify['id'] }}" @if(!empty($info['extend']['classify_id']) && $info['extend']['classify_id'] == $classify['id']) selected @endif>{{ $classify['name'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-xs-12 col-sm-10">
                    <label>
                        <input type="radio" name="row[status]" value="1" class="flat-red" @if ($info->status == 1) checked @endif> 开启
                    </label>
                    <label>
                        <input type="radio" name="row[status]" value="2" class="flat-red" @if ($info->status == 2) checked @endif> 关闭
                    </label>
                </div>
            </div>
            <div class="form-group row">
