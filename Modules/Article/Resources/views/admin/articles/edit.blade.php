        ...
        <form id="add-form" class="form-horizontal form-ajax card-body" role="form" data-toggle="validator" method="POST" action="">
            ...
            <div class="form-group row g-lg-2 g-1">
                ...
                <div class="col-xs-12 col-sm-10">
                    <input type="text" class="form-control" id="title" name="row[title]" placeholder="" value="{{$info->title??''}}" data-rule="required" />
                </div>
            </div>
                ...
                <div class="col-xs-12 col-sm-10">
                    <select class="form-control custom-select  col-xs-12 col-sm-12" name="row[classify_id]" style="border-radius:0px;" >
                        ...
                        @foreach ($classify_list as $classify)
                            <option value="{{ $classify['id'] }}" @if($info['classify_id']==$classify['id']) selected @endif>{{ $classify['name'] }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group row g-lg-2 g-1">
                ...
                <div class="col-xs-12 col-sm-10">
                    <input type="text" class="form-control" id="source_url" name="row[source_url]" placeholder="" value="{{$info->source_url??''}}" data-rule="" />
                </div>
            </div>
            <div class="border-top border-dashed my-2"></div>
            <div class="form-group row g-lg-2 g-1">
                ...
                <div class="col-xs-12 col-sm-10">
                    <input type="text" class="form-control" id="author" name="row[author]" placeholder="" value="{{$info->author??''}}" data-rule="" />
                </div>
            </div>
            <div class="border-top border-dashed my-2"></div>
            <div class="form-group row g-lg-2 g-1">
                ...
                <div class="col-xs-12 col-sm-12">
                    {{-- 编辑器容器--}}
                    <div class="summernote form-control col-xs-12 col-sm-12">{!! $info['content'] !!}</div>
                    <input type="hidden" name="row[content]" id="edit_content" value="{{$info['content']??''}}">
                </div>
            </div>
            <div class="border-top border-dashed my-2"></div>
            <div class="form-group row g-lg-2 g-1">
