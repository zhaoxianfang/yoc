...
use Modules\Core\Constants\SystemEmails;
use Modules\Core\Contracts\MiddlewareAbstract;
use Modules\Logs\Models\Blacklist;
...
class SecurityMiddleware extends MiddlewareAbstract
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        // 3. 检查速率限制（防止暴力破解）
        if ($this->checkRateLimitExceeded($request, $ip)) {
            return $this->handleSecurityViolation(
                $request,
                'RateLimit',
                '请求频率过高',
                '系统检测到异常高频访问',
                ['ip' => $ip, 'rate' => $this->getCurrentRate($ip)]
            );
        }

        // 4. 检查黑名单记录
        if (Schema::hasTable('blacklists')) {
            $blackInfo = $this->getBlacklistInfo($userId, $ip);
            if ($this->shouldBlockRequest($blackInfo, $isLocalIp)) {
                return $this->handleSecurityViolation(
                    $request,
                    'Forbidden',
                    '黑名单拦截',
                    '您的IP或账户已被封禁',
                    ['blacklist' => $blackInfo->toArray()]
                );
            }
        }
    ...
    protected function checkBannedIps(Request $request): void
    {
        ...
        foreach ($banIpArr as $ipPattern) {
            if ($this->matchIpPattern($userIp, $ipPattern)) {
                $this->handleSecurityViolation(
                    $request,
                    'IpBlocked',
                    'IP黑名单拦截',
                    '您的IP已被禁止访问',
                    ['ip' => $userIp, 'pattern' => $ipPattern]
                );
            }
        }
    }
            ...
            if (is_string($value)) {
                foreach (self::MALICIOUS_PATTERNS as $pattern) {
                    ...
                    if (preg_match($pattern, $value) &&
                        ! $this->isFalsePositive($request, $key, $value)) {
     ...
     * @param  string  $ip  客户端IP
     * @return Blacklist|null 黑名单记录
     */
    protected function getBlacklistInfo(?int $userId, string $ip): ?Blacklist
    {
        $query = Blacklist::query()
            ->select([
                'id', 'user_id', 'ip', 'remark', 'visits', 'type',
                'ban_deadline', 'ban_duration', 'ban_number',
                'created_at', 'updated_at',
            ]);

        return $userId
            ? $query->where('user_id', $userId)->first()
            : $query->where('ip', $ip)->first();
    }

    /**
     * 检查是否应该拦截请求
     *
     * @param  Blacklist|null  $blackInfo  黑名单记录
     * @param  bool  $isLocalIp  是否本地IP
     * @return bool 是否拦截
     */
    protected function shouldBlockRequest(?Blacklist $blackInfo, bool $isLocalIp): bool
    {
        return ! $isLocalIp && $blackInfo && $blackInfo->ban_deadline &&
               Carbon::parse($blackInfo->ban_deadline)->gte(now());
    }
     ...
     * @return bool 是否超出限制
     */
    protected function checkRateLimitExceeded(Request $request, string $ip): bool
    {
        $key = 'rate_limit:'.$ip.':'.$request->path();
        $maxAttempts = config('blacklist.rate_limit', 60);
        $decaySeconds = config('blacklist.rate_limit_decay', 60);

        if (RateLimiter::tooManyAttempts($key, $maxAttempts)) {
            $this->logInterception(
                $request,
                'RateLimitExceeded',
                [
                    'ip' => $ip,
    ...
    protected function handleSecurityViolation(
        ...
        // 更新黑名单（如果需要）
        $blackInfo = $this->updateBlacklist($request, $type);

        // 发送安全警报（如果需要）
        if ($this->shouldSendAlert($type)) {
            $this->sendSecurityAlert($request, $type, $title, $context, $blackInfo);
        }

        // 返回适当的响应
        return $this->createSecurityResponse($request, $type, $title, $message, $blackInfo);
    }
    ...
    protected function logSecurityEvent(
        ...
        // 使用独立的安全日志通道
        Log::channel('security')->warning("安全拦截: {$title}", $logData);
    }
     ...
     * @return Blacklist|null 黑名单记录
     */
    protected function updateBlacklist(Request $request, string $type): ?Blacklist
    {
        // 本地请求不加入黑名单
        if ($this->isLocalIp($request->ip())) {
            return null;
        }
        ...
        // 获取或创建黑名单记录
        $blackInfo = Blacklist::firstOrNew([
            $userId ? 'user_id' : 'ip' => $userId ?: $ip,
        ]);

        // 更新记录信息
        $blackInfo->type = $this->getBlacklistType($type);
        $blackInfo->remark = $this->getBlacklistRemark($type);
        $blackInfo->visits = ($blackInfo->visits ?? 0) + 1;
        $blackInfo->ban_number = ($blackInfo->ban_number ?? 0) + 1;

        // 设置封禁时间
        if ($this->shouldBan($type)) {
            $banDuration = $this->getBanDuration($type, $blackInfo->ban_number ?? 1);
            $blackInfo->ban_duration = $banDuration;
            $blackInfo->ban_deadline = now()->addHours($banDuration);
        }
     ...
     * @return int 黑名单类型常量
     */
    protected function getBlacklistType(string $violationType): int
    {
        return match ($violationType) {
            'Malicious', 'Anomalous' => Blacklist::TYPE_INTERCEPT,
            'RateLimit', 'Suspicious' => Blacklist::TYPE_SUSPICIOUS,
            default => Blacklist::TYPE_SUSPICIOUS
        };
    }
     ...
     * @return string 备注信息
     */
    protected function getBlacklistRemark(string $violationType): string
    {
        return match ($violationType) {
            'Malicious' => '恶意请求拦截',
            'Anomalous' => '异常参数拦截',
    ...
    protected function sendSecurityAlert(
        ...
        try {
            $data = [
                ...
                'blacklist_info' => $blackInfo ? $blackInfo->toArray() : null,
                'context' => $context,
            ];
    ...
    protected function createSecurityResponse(
        ...
        $responseData = [
            ...
            'type' => $type,
            'status' => $statusCode,
            'ban_deadline' => $blackInfo ? $blackInfo->ban_deadline : null,
