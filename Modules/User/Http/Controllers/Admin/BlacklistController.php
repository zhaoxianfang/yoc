...
use Modules\User\Models\Blacklist;
...
class BlacklistController extends AdminBaseController
{
    ...
    {
        if (! is_ajax()) {
            return view('user::admin.blacklist');
        }
        $req = $request->input();
    ...
    {
        if (! is_ajax()) {
            return view('user::admin.blacklist.add');
        }
        $request->validate([
            'row' => 'required|array',
            'row.ip' => 'required',
        ...
        try {
            ...
            return $this->success([], route('admin.user.blacklist.list'));
    ...
    {
        if (! is_ajax()) {
            return view('user::admin/blacklist/edit', [
                'info' => $blacklist,
            ]);
        }
        $request->validate([
